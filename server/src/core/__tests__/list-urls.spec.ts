...
import { isSuccess, isError, unwrapEither, Success, Error } from "@/shared/either";
import { URLTableModel } from "@/infra/db/schemas/url";
import { UnknownError } from "@/infra/http/errors/unknown-error";
import { randomUUID } from "node:crypto";

// Helper to insert a URL row for testing
...
describe("list-urls", () => {
  ...
  it("should return UnknownError if the database throws", async () => {
    ...
    const error = unwrapEither(result) as Error<UnknownError>["left"];  
